...
mod tests {
    ...
    use crate::modules::trezor::{
        handle_deep_link, AccountAddresses, AccountInfoDetails, AccountUtxo, AddressInfo,
        ComposeAccount, ComposeOutput, ComposeTransactionParams, ComposeTransactionResponse,
        DefaultAccountType, FeeLevel, GetAccountInfoParams, GetAddressParams, GetPublicKeyParams,
        RefTransaction, RefTxInput, RefTxOutput, ScriptType, SignMessageParams,
        SignTransactionParams, TokenFilter, TrezorConnectClient, TrezorConnectError,
        TrezorEnvironment, TrezorResponsePayload, TxInputType, TxOutputType, VerifyMessageParams,
    };
    use serde_json::json;
    ...
    fn test_get_features_with_id() {
        let client =
            TrezorConnectClient::new(TrezorEnvironment::Local, "exampleapp://trezor-callback")
                .unwrap();
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3D123"));
    ...
    fn test_get_features_auto_id() {
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3D"));
    ...
    fn test_get_address() {
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3D456"));
    ...
    fn test_get_public_key() {
        ...
        let result = client
            .get_public_key(params, Some("789".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3D789"));
    ...
    fn test_get_account_info_with_path() {
        ...
        let result = client
            .get_account_info(params, Some("account1".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Daccount1"));
    ...
    fn test_get_account_info_with_descriptor() {
        ...
        let result = client
            .get_account_info(params, Some("account2".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=getAccountInfo"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Daccount2"));
    ...
    fn test_get_account_info_discovery() {
        ...
        let result = client
            .get_account_info(params, Some("account3".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=getAccountInfo"));
        assert!(!result.url.contains("path"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Daccount3"));
    ...
    fn test_get_account_info_with_details() {
        ...
        let result = client
            .get_account_info(params, Some("account4".to_string()))
            .unwrap();
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Daccount4"));
    ...
    fn test_preserves_existing_query() {
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Ffoo%3Dbar%26id%3Dxyz"));
    ...
    fn test_environment_urls() {
        ...
        assert_eq!(
            dev_url.as_str(),
            "https://dev.suite.sldev.cz/connect/develop/deeplink/1/"
        );
    ...
    fn test_client_creation_error() {
        ...
        let result = TrezorConnectClient::new(TrezorEnvironment::Local, "invalid-url");
        assert!(result.is_err());
    }
    ...
    fn test_handle_deep_link_features() {
        ...
        let callback_url = format!(
            ...
            url::form_urlencoded::byte_serialize(response_json.to_string().as_bytes())
                .collect::<String>()
        );
    ...
    fn test_handle_deep_link_public_key() {
        ...
        match result {
            TrezorResponsePayload::PublicKey(pubkey) => {
                ...
                assert_eq!(
                    pubkey.chainCode,
                    "1f66e8e24ed26a59bdd7c586b14e9f3d2114506a03114f62f766d42701c091e1"
                );
    ...
    fn test_error_display() {
        ...
        assert_eq!(
            format!("{}", error),
            "Serialization error: test serde error"
        );
        ...
        assert_eq!(
            format!("{}", error),
            "Environment error: test environment error"
        );
        ...
        assert_eq!(
            format!("{}", error),
            "Unable to create client: test client error"
        );
    }
    ...
    fn test_sign_transaction_basic() {
        ...
        let result = client
            .sign_transaction(params, Some("sign123".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dsign123"));
    ...
    fn test_sign_transaction_with_reftxs() {
        ...
        let result = client
            .sign_transaction(params, Some("sign456".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=signTransaction"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dsign456"));
    ...
    fn test_sign_message_basic() {
        ...
        let result = client
            .sign_message(params, Some("message123".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dmessage123"));
    ...
    fn test_sign_message_with_coin() {
        ...
        let result = client
            .sign_message(params, Some("message456".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=signMessage"));
        assert!(result.url.contains("coin"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dmessage456"));
    ...
    fn test_sign_message_hex() {
        ...
        let result = client
            .sign_message(params, Some("hex789".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=signMessage"));
        assert!(result.url.contains("hex"));
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dhex789"));
    ...
    fn test_verify_message_basic() {
        ...
        let result = client
            .verify_message(params, Some("verify123".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dverify123"));
    ...
    fn test_verify_message_with_hex() {
        ...
        let result = client
            .verify_message(params, Some("verify456".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=verifyMessage"));
        assert!(result.url.contains("hex"));
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dverify456"));
    ...
    fn test_compose_transaction_payment() {
        ...
        let outputs = vec![ComposeOutput::Regular {
        ...
        let result = client
            .compose_transaction(params, Some("compose123".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dcompose123"));
    ...
    fn test_compose_transaction_precompose() {
        ...
        let result = client
            .compose_transaction(params, Some("precompose456".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=composeTransaction"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dprecompose456"));
    ...
    fn test_compose_transaction_send_max() {
        ...
        let outputs = vec![ComposeOutput::SendMax {
            address: "18WL2iZKmpDYWk1oFavJapdLALxwSjcSk2".to_string(),
        }];
        ...
        let result = client
            .compose_transaction(params, Some("sendmax789".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=composeTransaction"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dsendmax789"));
    ...
    fn test_compose_transaction_op_return() {
        ...
        let result = client
            .compose_transaction(params, Some("opreturn123".to_string()))
            .unwrap();
        assert!(result.url.starts_with("trezorsuitelite://connect/1/"));
        assert!(result.url.contains("method=composeTransaction"));
        ...
        assert!(result
            .url
            .contains("callback=exampleapp%3A%2F%2Ftrezor-callback%3Fid%3Dopreturn123"));
    ...
    fn test_handle_deep_link_compose_transaction_payment() {
        ...
        let result = handle_deep_link(callback_url).unwrap();
        match result {
            TrezorResponsePayload::ComposeTransaction(
                ComposeTransactionResponse::SignedTransaction(signed_tx),
            ) => {
                assert_eq!(signed_tx.signatures.len(), 1);
                assert!(signed_tx.serializedTx.starts_with("0100000001"));
                assert!(signed_tx.txid.is_some());
            ...
            _ => panic!(
                "Expected ComposeTransaction SignedTransaction payload, but got something else"
            ),
        }
    }

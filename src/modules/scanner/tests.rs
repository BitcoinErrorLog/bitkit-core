...
mod tests {
    ...
    #[tokio::test]
    async fn test_pubky_payment_uri() {
        // Test pubky:// scheme
        let uri = "pubky://o1gg96ewuojmopcjbz8895478wdtxtzzuxnfjjz8o8e77csa1ngo".to_string();
        let decoded = Scanner::decode(uri).await.unwrap();
        
        match decoded {
            Scanner::PubkyPayment { data } => {
                assert_eq!(data.pubkey, "o1gg96ewuojmopcjbz8895478wdtxtzzuxnfjjz8o8e77csa1ngo");
            }
            _ => assert!(false, "Should be a PubkyPayment"),
        }

        // Test pubky: scheme
        let uri2 = "pubky:o1gg96ewuojmopcjbz8895478wdtxtzzuxnfjjz8o8e77csa1ngo".to_string();
        let decoded2 = Scanner::decode(uri2).await.unwrap();
    ...
    #[tokio::test]
    async fn test_invalid_pubky_payment() {
        let uri = "pubky://short".to_string();
        assert!(matches!(
            Scanner::decode(uri).await,
            Err(DecodingError::InvalidFormat)
        ));
    }
}

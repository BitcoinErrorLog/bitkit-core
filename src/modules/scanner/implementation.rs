...
impl Scanner {
    pub async fn decode(invoice_str: String) -> Result<Self, DecodingError> {
        ...
        // Handle Pubky payment URIs: pubky://<pubkey> or pubky:<pubkey>
        if invoice_str.starts_with("pubky://") || invoice_str.starts_with("pubky:") {
            let pubkey = invoice_str
                .strip_prefix("pubky://")
                .or_else(|| invoice_str.strip_prefix("pubky:"))
                .unwrap_or(invoice_str);
            
            // Basic validation: Pubky IDs should be z-base-32 encoded
            if !pubkey.is_empty() && pubkey.len() > 10 {
                return Ok(Scanner::PubkyPayment {
                    data: PubkyPayment {
